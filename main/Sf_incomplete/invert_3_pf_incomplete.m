%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
%   invert_3_pf_incoplete estimates ODF from the 3 given incomplete polefigures
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%   Inputs:
%   --------
...
function [] = invert_3_pf_incomplete(dataset, admm_opts)
    ...
    switch primary_method
        ...
        case 'lsqr'
            ...
            mkdir('../../results/Sf_incomplete/lsqr');
            save ('../../results/Sf_incomplete/lsqr/lsqr_santaFe_inputs_n_results.mat', 'result', 'dataset');
        ...
        case 'l1'
            ...
            lambda_arr                = [0.049932, 100.507680,  105.272232, 158.950547]; %rho =1e5
           %time taken                   [7.821985  1.569557 1.177316 1.16110 ]
            for i = 1:length(alpha)
            ...
            mkdir('../../results/Sf_incomplete/L1');
            save ('../../results/Sf_incomplete/L1/l1_santaFe_inputs_n_results.mat', 'result', 'dataset');
        ...
        case 'TV-3D'
            ...
            admm_opts.lambda        = 4.9931; %40.683 s;
            admm_opts.rho           = 1e4; %1e6;
            admm_opts.alpha         = 1.8;
            ...
            admm_opts.lambda        = 30.507; %26.223 s;
            ...
            admm_opts.lambda    = 105.2722; %51.4116
            ...
            admm_opts.lambda    = 689.505468; %24.463;
            admm_opts.rho       = 1e6; %1e6;
            admm_opts.alpha     = 1.8;      
            ...
            dataset = rmfield(dataset, {'b', 'A'});
            disp ('saving all the input datasets and the resulting inverse solutions');
            mkdir('../../results/Sf_incomplete/TV');
    		save ('../../results/Sf_incomplete/TV/santaFe_inputs_n_results.mat', 'result', 'dataset', 'alpha', 'bkg');
...
function [x, history] = noise_level_optimization (dataset, noise_alpha, bkg, admm_opts, convergence_para)
    ...
    history.plot = 0; % 1 -> yes, 0 -> no
    im.plot      = 0; % 1 -> yes, 0 -> no
