...
%   & RMSE (Root Mean Squared Error) values at each iteration. 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k = 1:MAX_ITER
    ...
    history.cost(k)  = costFunc(A, b, lambda, x, F1, F2, F3, F1b, F2b, F3b);
   ...
   % reporting 
    if strcmpi (convergence_para.disp, 'True') 
        fprintf('%3d\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10f\n', k, ...
...
function p = costFunc(A, b, lambda, x, F1, F2, F3, F1b, F2b, F3b)
    p = ( 1/2*sum((A*x - b).^2) + lambda*norm(F1*x,1) + lambda*norm(F2*x,1) ...
        + lambda*norm(F3*x,1) + lambda*norm(F1b*x,1) + lambda*norm(F2b*x,1) ...
        + lambda*norm(F3b*x,1));
