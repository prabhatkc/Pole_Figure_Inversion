...
% read angles
angles = load('incomplete_angles.txt');
...
nlevels = length(dataset.alpha);
pf_opt  = zeros(m, nlevels); % reconstructed PF from reconstructed ODF for 5 errror levels;

for i = 1:nlevels
    x=result.all_recon(:, i);
    pf_opt(:, i)  = A*x;
end
...
for i = 1:4
    for j = 1:3
        ...
        fid = fopen(fname,'w');
        for k = 1:Nt
            fprintf(fid,'%f\t%f\t%f\n',angles(k,1),angles(k,2),pf_opt((j-1)*Nt+k,i));
        end
        fclose(fid);
    end
end
