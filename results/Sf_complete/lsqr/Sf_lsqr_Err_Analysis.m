...
load('lsqr_santaFe_inputs_n_results.mat');
...
for i = 1:nlevels
    pf_string.title     = sprintf(['LSQR resolved ODF, initially, corrupted with poisson noise a = ', num2str(alpha(i)), ' & b = ', num2str(bkg)]);
    figure, suptitle(pf_string.title);
    plotLayers(reshape(recon_odf(:, i), Xdim)); set(gcf, 'Position', [100, 100, 800, 750]);
    out_filename = sprintf(['lsqr_recovered_odf_a_',num2str(alpha(i)),'_b_',num2str(bkg), '.eps']);
...
print -depsc2 odf_err_lsqr_sf_complete.eps
close;
...
print -depsc2 pf_snr_lsqr_sf_complete.eps
close;
...
print -depsc2 pf_psnr_lsqr_sf_complete.eps
close;
